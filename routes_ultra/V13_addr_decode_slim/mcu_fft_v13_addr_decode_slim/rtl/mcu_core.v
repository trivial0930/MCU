...
module mcu_core(
    ...
    wire signed [24:0] rdata1;
    wire signed [24:0] rdata2;
    reg  signed [24:0] reg_wdata;
    reg  [3:0] reg_waddr;
    reg  reg_we_final;
    ...
    reg [3:0] alu_op;
    reg signed [24:0] alu_a;
    reg signed [24:0] alu_b;
    wire signed [24:0] alu_y;
    wire alu_zero;
    wire alu_negative;

    wire [7:0] mem_offset8;
    wire [4:0] mem_offset5;
    wire [1:0] mem_region;
    wire is_test_rom;
    wire is_verify_ram;
    ...
    assign exec_instr = instr_id;
    assign cmp_zero = (rdata1[24:0] == rdata2[24:0]);
    ...
    assign mem_offset8 = rdata1[7:0] + imm16[7:0];
    assign mem_offset5 = mem_offset8[4:0];
    assign mem_region =
        (rdata1[15:8] == 8'h10) ? `MEM_REGION_TEST :
        (rdata1[15:8] == 8'h20) ? `MEM_REGION_VERIFY :
                                  `MEM_REGION_DATA;
    assign is_test_rom = (mem_region == `MEM_REGION_TEST);
    assign is_verify_ram = (mem_region == `MEM_REGION_VERIFY);

    assign dmem_addr = {8'd0, mem_offset8};
    assign dmem_wdata = rdata2[15:0];
    assign dmem_we = ctrl_mem_write && (mem_region == `MEM_REGION_DATA) && !done;
    assign internal_read_data = {{9{dmem_rdata[15]}}, dmem_rdata};

    ext_test_rom_if u_ext_test_rom_if(
        .is_test_rom(is_test_rom),
        .test_offset(mem_offset8),
    ...
    verify_ram_if u_verify_ram_if(
        .is_verify_ram(is_verify_ram),
        .verify_offset(mem_offset5),
