...
CAPTURE = BOARD / "v59_ila_fast_stop_capture.csv"
EXPECTED = ROOT / "results" / "expected_fft_output.txt"
OUT_COMPARE = BOARD / "v59_hw_compare.csv"
OUT_PROOF = BOARD / "v59_fast_stop_proof.csv"
STATUS = BOARD / "v59_hw_compare_status.txt"
...
def main():
    ...
    idx_owner_seen = find_column(header, "owner_seen_dbg")
    idx_owner_done = find_column(header, "owner_done_dbg")
    ...
    for row in rows[2:]:
        ...
        owner_seen = parse_int(row[idx_owner_seen])
        owner_done = parse_int(row[idx_owner_done])
        ...
        for core in range(8):
            ...
            writes.append((sample, cnt, core, addr, data, we_mask, done, done_mask, owner_seen, owner_done, fast_stop))
    ...
    for addr in range(16):
        ...
        if status != "PASS":
            compare_ok = False
        compare_rows.append([addr, data, expected[addr], cnt, sample, core, f"0x{we_mask:02x}", int(done), status])
    ...
    first_stop_owner_done = first_fast_stop["owner_done"] if first_fast_stop else None
    first_stop_owner_seen = first_fast_stop["owner_seen"] if first_fast_stop else None
    first_stop_we_mask = first_fast_stop["we_mask"] if first_fast_stop else None

    done_mask_after_stop_sample = None
    owner_done_after_stop_sample = None
    if first_fast_stop is not None:
        current_verify_bits = 0
        for item in writes:
            if item[0] == first_fast_stop["sample"]:
                current_verify_bits |= 1 << item[3]
        done_mask_after_stop_sample = first_fast_stop["done_mask"] | current_verify_bits
        owner_done_after_stop_sample = first_fast_stop["owner_done"] | (
            first_fast_stop["we_mask"] & first_fast_stop["owner_seen"]
        )
    ...
    fast_stop_not_early = (
        ...
        and done_mask_after_stop_sample == 0xFFFF
        and owner_done_after_stop_sample == 0xFF
        and first_stop_owner_seen == 0xFF
    )
    ...
    proof_rows = [
        ...
        ["verify_we_at_first_fast_stop", f"0x{first_stop_we_mask:02x}" if first_stop_we_mask is not None else ""],
        ...
        ["verify_done_mask_next_at_first_fast_stop", f"0x{done_mask_after_stop_sample:04x}" if done_mask_after_stop_sample is not None else ""],
        ["owner_seen_at_first_fast_stop", f"0x{first_stop_owner_seen:02x}" if first_stop_owner_seen is not None else ""],
        ["owner_done_q_at_first_fast_stop", f"0x{first_stop_owner_done:02x}" if first_stop_owner_done is not None else ""],
        ["owner_done_next_at_first_fast_stop", f"0x{owner_done_after_stop_sample:02x}" if owner_done_after_stop_sample is not None else ""],
    ...
    summary = [
        ...
        f"verify_we_at_first_fast_stop=0x{first_stop_we_mask:02x}" if first_stop_we_mask is not None else "verify_we_at_first_fast_stop=",
        ...
        f"verify_done_mask_next_at_first_fast_stop=0x{done_mask_after_stop_sample:04x}" if done_mask_after_stop_sample is not None else "verify_done_mask_next_at_first_fast_stop=",
        f"owner_seen_at_first_fast_stop=0x{first_stop_owner_seen:02x}" if first_stop_owner_seen is not None else "owner_seen_at_first_fast_stop=",
        f"owner_done_q_at_first_fast_stop=0x{first_stop_owner_done:02x}" if first_stop_owner_done is not None else "owner_done_q_at_first_fast_stop=",
        f"owner_done_next_at_first_fast_stop=0x{owner_done_after_stop_sample:02x}" if owner_done_after_stop_sample is not None else "owner_done_next_at_first_fast_stop=",
        ...
        "compare_csv=board_validation/v59_hw_compare.csv",
        "proof_csv=board_validation/v59_fast_stop_proof.csv",
    ]
    STATUS.write_text("\n".join(summary) + "\n", encoding="utf-8")
    print("\n".join(summary))
    if not pass_status:
