...
module mcu_core(
    ...
    wire signed [31:0] rf_rdata1;
    wire signed [31:0] rf_rdata2;
    wire signed [31:0] operand1_id;
    wire signed [31:0] operand2_id;
    ...
    reg [15:0] ex_imm16;
    reg [31:0] ex_imm32;
    reg signed [31:0] ex_op1;
    reg signed [31:0] ex_op2;
    reg [15:0] ex_pc;
    reg ex_reg_we;
    reg ex_mem_read;
    reg ex_mem_write;
    ...
    wire signed [31:0] test_rom_read_data;
    wire signed [31:0] internal_read_data;
    wire signed [31:0] alu_fast_y;
    wire ex_forward_can_bypass;
    wire [3:0] ex_forward_rd_waddr;
    wire signed [31:0] ex_forward_wdata;
    wire ex_fwd_rs1_hit;
    wire ex_fwd_rs2_hit;
    wire wb_fwd_rs1_hit;
    wire wb_fwd_rs2_hit;
    ...
    reg [3:0] mul_count;
    reg [41:0] mul_acc;
    reg [41:0] mul_multiplicand;
    reg [7:0] mul_multiplier;
    reg mul_neg;
    reg [3:0] mul_waddr;
    reg mul_final;

    wire signed [32:0] ex_add = {ex_op1[31], ex_op1} + {ex_op2[31], ex_op2};
    wire signed [32:0] ex_sub = {ex_op1[31], ex_op1} - {ex_op2[31], ex_op2};
    wire [32:0] ex_abs_a = ex_op1[31] ? (~{ex_op1[31], ex_op1} + 33'd1) : {ex_op1[31], ex_op1};
    wire [7:0] ex_abs_b = ex_op2[7] ? (~ex_op2[7:0] + 8'd1) : ex_op2[7:0];
    wire [41:0] mul_x1 = mul_multiplicand;
    wire [41:0] mul_x2 = mul_multiplicand << 1;
    wire [41:0] mul_x4 = mul_multiplicand << 2;
    wire [41:0] mul_x8 = mul_multiplicand << 3;
    wire [41:0] mul_addend =
        (mul_multiplier[0] ? mul_x1 : 42'd0) +
        (mul_multiplier[1] ? mul_x2 : 42'd0) +
        (mul_multiplier[2] ? mul_x4 : 42'd0) +
        (mul_multiplier[3] ? mul_x8 : 42'd0);
    wire [41:0] mul_acc_next = mul_acc + mul_addend;
    wire signed [41:0] mul_signed_mag = mul_neg ? -$signed(mul_acc) : $signed(mul_acc);
    wire signed [31:0] mul_result_q7 = (mul_signed_mag >>> 7);
    ...
    wire stall_id = raw_hazard_ex || mul_busy || mul_final || ex_mul_will_write;
    ...
    assign ex_mem_offset8 = ex_imm16[7:0];
    ...
    assign ex_forward_wdata =
        (ex_opcode == `OP_MOVI) ? ex_imm32 :
        (ex_opcode == `OP_BL)   ? ({16'd0, ex_pc} + 32'd1) :
                                  alu_fast_y;
    assign cmp_zero_ex = (ex_op1 == ex_op2);
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (mul_final) begin
                ...
                if (ex_opcode == `OP_MUL) begin
                    mul_busy <= 1'b1;
                    mul_count <= 4'd0;
                    mul_acc <= 42'd0;
                    mul_multiplicand <= {9'd0, ex_abs_a};
                    mul_multiplier <= ex_abs_b;
                    mul_neg <= ex_op1[31] ^ ex_op2[7];
                    mul_waddr <= ex_rd;
                end else begin
                    wb_valid <= ex_reg_we;
                    wb_we <= ex_reg_we;
