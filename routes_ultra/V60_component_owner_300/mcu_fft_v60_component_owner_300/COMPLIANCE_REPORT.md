...
V60 的速度来自任务拆分，而不是新增硬件算子。16 个 core 的结构相同，均包含 PC、指令 ROM、decoder、寄存器堆、ALU、load/store、writeback 和 halt。每个 core 从自己的指令 ROM 中取 32-bit 指令，按普通 MCU 流程执行。

实际执行的指令类型可在 `results/opcode_summary_all.csv` 中查看，包括：
...
4. 正式回归和板上 `done` 稳定后读到 `cnt_test=38`。

因此 `cnt_test=38` 不是通过少写地址、跳过 addr15 或提前停表得到的。板上 ILA 证明见 `board_validation/v60_fast_stop_proof.csv`。
