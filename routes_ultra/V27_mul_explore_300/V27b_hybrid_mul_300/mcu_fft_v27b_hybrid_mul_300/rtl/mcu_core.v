...
module mcu_core(
    ...
    wire ex_mul_fast_const =
        (ex_abs_b == 8'd0)   || (ex_abs_b == 8'd1)  ||
        (ex_abs_b == 8'd2)   || (ex_abs_b == 8'd4)  ||
        (ex_abs_b == 8'd8)   || (ex_abs_b == 8'd16) ||
        (ex_abs_b == 8'd32)  || (ex_abs_b == 8'd64) ||
        (ex_abs_b == 8'd91)  || (ex_abs_b == 8'd128);
    wire [33:0] ex_mul_c91_mag = (ex_mul_base << 7) - (ex_mul_base << 5) -
                                  (ex_mul_base << 2) - ex_mul_base;
    wire [33:0] ex_mul_fast_mag =
        (ex_abs_b == 8'd0)   ? 34'd0 :
        (ex_abs_b == 8'd1)   ? ex_mul_base :
        (ex_abs_b == 8'd2)   ? (ex_mul_base << 1) :
        (ex_abs_b == 8'd4)   ? (ex_mul_base << 2) :
        (ex_abs_b == 8'd8)   ? (ex_mul_base << 3) :
        (ex_abs_b == 8'd16)  ? (ex_mul_base << 4) :
        (ex_abs_b == 8'd32)  ? (ex_mul_base << 5) :
        (ex_abs_b == 8'd64)  ? (ex_mul_base << 6) :
        (ex_abs_b == 8'd91)  ? ex_mul_c91_mag :
        (ex_abs_b == 8'd128) ? (ex_mul_base << 7) :
                                34'd0;
    wire signed [33:0] ex_mul_fast_signed_mag =
        (ex_op1[24] ^ ex_op2[7]) ? -$signed(ex_mul_fast_mag) : $signed(ex_mul_fast_mag);
    wire signed [24:0] ex_mul_fast_result_q7 = (ex_mul_fast_signed_mag >>> 7);
    ...
    wire ex_mul_fast_will_write = ex_mul_will_write && ex_mul_fast_const;
    wire ex_mul_slow_will_write = ex_mul_will_write && !ex_mul_fast_const;
    ...
    wire stall_id = raw_hazard_ex || mul_busy || mul_final || ex_mul_slow_will_write;
        ...
        if (rst) begin
            ...
            if (mul_final) begin
                ...
                if (ex_opcode == `OP_MUL) begin
                    if (ex_mul_fast_const) begin
                        wb_valid <= ex_reg_we;
                        wb_we <= ex_reg_we;
                        wb_waddr <= ex_rd;
                        wb_wdata <= ex_mul_fast_result_q7;
                    end else begin
                        mul_busy <= 1'b1;
                        mul_count <= 4'd0;
                        mul_acc <= 34'd0;
                        mul_multiplicand <= {8'd0, ex_abs_a};
                        mul_multiplier <= ex_abs_b;
                        mul_neg <= ex_op1[24] ^ ex_op2[7];
                        mul_waddr <= ex_rd;
                    end
                end else begin
                    wb_valid <= ex_reg_we;
