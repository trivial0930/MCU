...
CAPTURE = BOARD / "v34_ila_verify_we_capture.csv"
EXPECTED = ROOT / "results" / "expected_fft_output.txt"
OUT = BOARD / "v34_hw_compare.csv"
STATUS = BOARD / "v34_hw_compare_status.txt"
...
def main():
    ...
    for row in data_rows:
        if len(row) < len(header):
            continue
        if row[idx["u_ila_probe/verify_we"]].strip().lower() not in {"1", "01", "1'b1"}:
            continue
    ...
    for row in data_rows:
        if len(row) < len(header):
            continue
        if row[idx["u_ila_probe/done"]].strip().lower() not in {"1", "01", "1'b1"}:
            continue
    ...
    summary = [
        ...
        f"compare_status={'PASS' if ok and len(writes) >= 16 and len(first_by_addr) == 16 else 'FAIL'}",
        "csv=board_validation/v34_hw_compare.csv",
    ]
    STATUS.write_text("\n".join(summary) + "\n", encoding="utf-8")
    print("\n".join(summary))
    if not (ok and len(writes) >= 16 and len(first_by_addr) == 16):
        raise SystemExit(1)
