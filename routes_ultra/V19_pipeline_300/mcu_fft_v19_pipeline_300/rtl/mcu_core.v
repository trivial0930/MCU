...
module mcu_core(
    ...
    wire id_uses_rs2 =
        ...
        (opcode_id == `OP_CMP);
    wire ex_will_write = ex_valid && ex_reg_we && (ex_opcode != `OP_MUL);
    wire ex_mul_will_write = ex_valid && (ex_opcode == `OP_MUL);
    wire raw_hazard_ex =
        ex_valid && (ex_reg_we || ex_mul_will_write) &&
    ...
    assign operand1_id =
        (wb_valid && wb_we && (wb_waddr == rs1_id)) ? wb_wdata : rf_rdata1;
    assign operand2_id =
        (wb_valid && wb_we && (wb_waddr == rs2_id)) ? wb_wdata : rf_rdata2;
