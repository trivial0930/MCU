...
set status_file [file join $base_dir "capture_v54_ila_status.txt"]
set bit_file [file normalize "D:/vivado_work/routes_ultra/mcu_fft_v54_octa_output_owner_300_ila/mcu_fft_board.runs/impl_1/board_top.bit"]
set ltx_file [file normalize "D:/vivado_work/routes_ultra/mcu_fft_v54_octa_output_owner_300_ila/mcu_fft_board.runs/impl_1/debug_nets.ltx"]
set csv_file [file join $base_dir "v54_ila_verify_we_capture.csv"]
...
if {[catch {
    ...
    set verify_we_probe [lindex [get_hw_probes *verify_we_all* -of_objects $ila] 0]
    if {$verify_we_probe eq ""} {
        error "cannot find verify_we_all probe"
    }
    set_property TRIGGER_COMPARE_VALUE {eq8'h55} $verify_we_probe

    log_line $fd "program_status=ok"
    log_line $fd "device=$dev"
    log_line $fd "ila=$ila"
    log_line $fd "armed_for=verify_we_all_eq_0x55"
    ...
    log_line $fd "csv=board_validation/v54_ila_verify_we_capture.csv"
} err]} {
