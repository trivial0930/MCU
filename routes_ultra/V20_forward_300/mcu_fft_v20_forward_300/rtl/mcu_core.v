...
module mcu_core(
    ...
    wire [33:0] mul_addend = mul_multiplier[0] ? mul_multiplicand : 34'd0;
    ...
    wire raw_hazard_ex =
        (ex_mul_will_write || ex_load_will_write) &&
        (((id_uses_rs1 && (rs1_id == ex_rd))) || ((id_uses_rs2 && (rs2_id == ex_rd))));
    ...
    assign operand1_id =
        (ex_forward_valid && (ex_forward_waddr == rs1_id)) ? ex_forward_wdata :
        (wb_valid && wb_we && (wb_waddr == rs1_id)) ? wb_wdata :
        rf_rdata1;
    assign operand2_id =
        (ex_forward_valid && (ex_forward_waddr == rs2_id)) ? ex_forward_wdata :
        (wb_valid && wb_we && (wb_waddr == rs2_id)) ? wb_wdata :
        rf_rdata2;
    ...
    assign alu_fast_y =
        ...
        (ex_opcode == `OP_MOVR) ? ex_op1 :
                                  ex_add[24:0];
    assign ex_forward_valid = ex_valid && ex_reg_we && (ex_opcode != `OP_MUL) && (ex_opcode != `OP_LDR);
    assign ex_forward_waddr = ex_link ? 4'd14 : ex_rd;
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            if (mul_final) begin
                ...
                mul_multiplier <= {1'b0, mul_multiplier[7:1]};
                if (mul_count == 4'd7) begin
                    mul_busy <= 1'b0;
                    mul_final <= 1'b1;
                end else begin
