...
ASM_FILES = []
for i in range(16):
    component = "real" if i < 8 else "imag"
    k = i if i < 8 else i - 8
    ASM_FILES.append((f"core{i}", ROOT / "asm" / f"fft8_core{i:02d}_{component}_x{k}.asm"))
FORBIDDEN_MODULES = [
    "fft_engine",
    "dft_engine",
    "butterfly_unit",
...
def main():
    ...
    (RESULTS / "octa_audit_summary.txt").write_text(
        f"cnt_test={cnt_test}\n"
        "core_count=16\n"
        "conclusion=COMPONENT_OWNER_FULL_MCU_COMPLIANCE_AUDIT_PASS\n",
        encoding="utf-8",
    )
    print("component-owner audit complete")
