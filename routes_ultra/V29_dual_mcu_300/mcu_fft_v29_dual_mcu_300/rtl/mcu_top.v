module mcu_top(
    ...
    instr_rom #(
        .INIT_FILE("mem/instr_idle.mem")
    ) u_instr_rom_core1(
        .addr(instr_addr_core1),
        .instr(instr_core1)
    );

    data_ram u_data_ram_core0(
        .clk(clk),
        .we(dmem_we_core0),
        .addr(dmem_addr_core0),
        .wdata(dmem_wdata_core0),
        .rdata(dmem_rdata_core0)
    );

    data_ram u_data_ram_core1(
        .clk(clk),
        .we(dmem_we_core1),
        .addr(dmem_addr_core1),
        .wdata(dmem_wdata_core1),
        .rdata(dmem_rdata_core1)
    );
