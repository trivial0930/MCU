...
- `routes_ultra/` 保存 300 MHz 极限路线，当前最快 timing-clean 候选为 `V26_scheduled_mul2_300` / `V28_branch_reduce_300`，已上板验证主线仍为 `V22b_fast_mul2_300`。
...
Ultra 最新结果：`routes_ultra/V22b_fast_mul2_300` 已在 300 MHz 下完成官方样例 + 20 组随机回归、Vivado 实现、DRC、bitstream 和实物上板验证，`cnt_test=173`，按 300 MHz 推算约 `0.577 us`，post-route WNS 为 `+0.122 ns`，DSP=0。V26/V28 通过输出基址前移把 `cnt_test` 降到 172，300 MHz timing-clean，理论时间约 `0.573 us`，但尚未单独上板。V27a/V27b 把功能周期降到 157，但 300 MHz 时序失败，作为负结果保留。V29 已搭建双完整 MCU Phase 1 骨架并 300 MHz timing-clean，但 Core1 目前为空闲程序，不作为速度主线。
...
3. `routes_ultra/README.md`：查看 300 MHz 极限路线、V26/V28/V29 状态和 bitstream 位置。
