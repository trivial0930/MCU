...
Ultra 最新结果：`routes_ultra/V22b_fast_mul2_300` 已在 300 MHz 下完成官方样例 + 20 组随机回归、Vivado 实现、DRC、bitstream 和实物上板验证，`cnt_test=173`，按 300 MHz 推算约 `0.577 us`，post-route WNS 为 `+0.122 ns`，DSP=0。ILA 抓波确认 16 个输出写回全部匹配 `FFT_output.coe`，最终板卡已切回无 ILA 正式版本。`routes_ultra/V24_load_forward_300` 已验证为负优化：功能 PASS 但 `cnt_test` 不降且 WNS 为 `-0.005 ns`，不推荐合并。`routes_ultra/V19_pipeline_300` 仍可作为 300 MHz 稳健保底路线。
