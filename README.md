...
本仓库用于电子科技大学英才实验学院数字电路 MCU FFT 项目。目标是在目标板卡 `xc7k160tffg676-2` 上，使用普通 32-bit ARM-like MCU 指令完成 8 点定点复数 FFT，并保留可复现的仿真、综合、实现、bitstream、上板验证和中文说明材料。
...
| V59 no-ILA 资源 | LUT 8677，FF 6451，DSP 0，BRAM 0 |
| V59 ILA 上板验证 | 已完成，16 次 verify 写回全覆盖，输出比对 PASS |
| V59 fast-stop 证明 | ILA 触发于 `fast_stop_pulse_dbg`，首次停表时 16 个 verify 地址均已可信写入，不是提前停表 |
...
- DSP 使用量为 0。
- `cnt_test` 保持全系统 wall-clock 口径，V59 已用 ILA 证明 fast-stop 没有通过少写 verify 或提前停表制造假加速。
