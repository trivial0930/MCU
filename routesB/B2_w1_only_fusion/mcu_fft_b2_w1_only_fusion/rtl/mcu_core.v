...
module mcu_core(
    ...
    assign is_ext_madd91 = (opcode == `OP_NOP) && (imm16 == `EXT_MADD91);
    assign is_ext_msub91 = (opcode == `OP_NOP) && (imm16 == `EXT_MSUB91);
    assign is_ext_mnsum91 = (opcode == `OP_NOP) && (imm16 == `EXT_MNSUM91);
    assign is_ext_math = is_ext_madd91 || is_ext_msub91 || is_ext_mnsum91;
    assign cmp_diff = $signed(rdata1) - $signed(rdata2);
    assign cmp_zero = (rdata1 == rdata2);
    assign cmp_negative = cmp_diff[31];
    ...
    always @(*) begin
        ...
        if (is_ext_madd91)
            alu_op = `ALU_MADD91;
        else if (is_ext_msub91)
            alu_op = `ALU_MSUB91;
        else if (is_ext_mnsum91)
            alu_op = `ALU_MNSUM91;
    end
    ...
    always @(*) begin
        reg_waddr = rd;
        reg_wdata = alu_y;
        reg_we_final = (ctrl_reg_we || is_ext_math) && !done;
